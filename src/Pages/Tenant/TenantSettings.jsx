...
const TenantSettings = () => {
  ...
  const posting = (e) => {
    const file = e.target.files[0];
    const myImage = URL.createObjectURL(file);
    setShowImg(myImage); // Show temporary image
    setFormData({ ...formData, profilePicture: file }); // Store file for uploading
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedProfile = JSON.parse(localStorage.getItem("userInfo"));
      updatedProfile.firstName = updatedTenant.firstName;
      updatedProfile.lastName = updatedTenant.lastName;
      updatedProfile.phoneNumber = updatedTenant.phoneNumber;
      updatedProfile.profilePicture = updatedTenant.profilePicture;
      localStorage.setItem("userInfo", JSON.stringify(updatedProfile));
